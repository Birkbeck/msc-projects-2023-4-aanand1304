...
def evaluate_model(train, test, n_input, layer, units, rdo, scaler, epochs):
    train_x, train_y = prepare_data(train, n_input)
    test_x, test_y = prepare_data(test, n_input)
    
    # Ensure we have enough data in test_x before reshaping
    if test_x.shape[0] == 0 or test_x.shape[1] == 0:
        raise ValueError(f"Insufficient test data for n_input: {n_input}. Test set size: {len(test)}")
