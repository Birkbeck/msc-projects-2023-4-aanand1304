...
attacks = [
    ...
    'Disinformation/Misinformation', 'Targeted Attack', 'Adware', 'Brute Force Attack',
    'Malvertising', 'Backdoor', 'Botnet', 'Cryptojacking', 'Worms', 'Spyware',
    'Unknown', 'Others'
]
...
datafile = open('Hackmageddon.csv', 'r', encoding='ISO-8859-1')
data = list(csv.reader(datafile))
data2 = []

for line in data:
    line[3] = line[3].strip()
    if len(line) == 4 and (len(line[3]) <= 2 or '-' in line[3]) and all(line):
        data2.append(line)

data = data2
print('Data has', len(data), 'rows')

malwares = ['MALWARE', 'WIPER', 'TROJAN', 'DROPPER', 'SPYWARE', 'PEGASUS', 'MALVERTIS', 'RANSOMWARE',
            'VIRUS', 'WORM', 'KEYLOG', 'KEYSTROKE LOG', 'MALICIOUS CODE', 'MALICIOUS SOFTWARE',
...
for d in data:
    h_date = d[0].strip()
    if len(h_date) == 8:
        h_date = h_date[:6] + '20' + h_date[6:]
    if int(h_date[3:5]) < 7 and int(h_date[6:10]) == 2011:
        continue
    if int(h_date[6:]) < 2023 or (int(h_date[6:]) == 2024 and int(h_date[3:5]) > 4):
        continue
...
with open('NoI_daily.csv', 'w', newline='') as f:
